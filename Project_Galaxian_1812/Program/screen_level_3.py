...
def screen_level_3(screen, dict_audio, menu, player):
    
    #Audio
    audio_level_3 = dict_audio.get_audio("audio_level_3")
    audio_level_3.play_audio(1.0, -1)
    ...
    #Character - Enemies
    final_boss = E4(IMAGE_ENEMY_4)
    enemy_1 = E1(IMAGE_ENEMY_1)
    enemy_2 = E2(IMAGE_ENEMY_2)
    enemy_3 = E3(IMAGE_ENEMY_3)  
    enemy_1.speed_x = random.choice([-8, -7])
    enemy_1.speed_y = random.choice([-8, -7])
    ...
    pygame.time.set_timer(timer_boss, 50 * 1000)
    
    #Program Execution
    running = True
    
    while running: 
        audio_level_3.check_mute()
        if player.no_lifes():
            ...
            list_e3.draw(screen)
            if input_boss:
                if list_boss.update(list_explosions, player, list_shoots_enemies, list_shoots_player):
                    audio_level_3.stop_audio()
                    running = False
                    return_level = True
                list_boss.draw(screen)
            ...
            audio_level_3.mute_icon(screen, image_audio_off, image_audio_on)
